...
for i in range(w, rows-w) :
    for j in range(w, cols-w) :
        a = np.amin(luvimg[i-w:i+w , j-w:j+w, 0])
        b = np.amax(luvimg[i-w:i+w , j-w:j+w, 0])
        eq_L = np.floor(((luvimg [i-w:i+w, j-w:j+w,0] - a )*(255/(b-a))) + 0.5)
        l, u, v = inputImage[i, j]
        m,n = eq_L.shape
        l = eq_L[math.floor(m/2)][math.floor(n/2)]
